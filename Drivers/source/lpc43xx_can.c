...
#define CAN_IF_Read(reg, IFsel) (LPC_C_CAN1->##IFsel##_##reg)
#define CAN_IF_Write(reg, IFsel, val) (LPC_C_CAN1->##IFsel##_##reg=val)

/* Macro for writing IF to specific RAM message object */
#define CAN_IF_readBuf(IFsel,msg) \
  LPC_C_CAN1->##IFsel##_##CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB; \
  LPC_C_CAN1->##IFsel##_##CMDREQ=msg; \
  while (LPC_C_CAN1->##IFsel##_##CMDREQ & IFCREQ_BUSY );

/* Macro for reading specific RAM message object to IF */
#define CAN_IF_writeBuf(IFsel,msg) \
  LPC_C_CAN1->##IFsel##_##CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB; \
  LPC_C_CAN1->##IFsel##_##CMDREQ=msg; \
  while (LPC_C_CAN1->##IFsel##_##CMDREQ & IFCREQ_BUSY );
#else
#define CAN_IF_Read(reg, IFsel) (LPC_C_CAN1->IFsel##_##reg)
...
//	if(IFsel == IF1){
//		return (LPC_C_CAN1->IF1_reg);
//	}else{
//		return (LPC_C_CAN1->IF2_reg);
//	}
//}
//void CAN_IF_Write(uint32_t reg, uint32_t IFsel,uint32_t val){
//	if(IFsel == IF1){
//	(LPC_C_CAN1->IF1_reg=val);
//	}else{
//		(LPC_C_CAN1->IF2_reg=val);
//	}
//}
//
///* Macro for writing IF to specific RAM message object */
...
//	if(IFsel == IF1){
//	LPC_C_CAN1->IF1_CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
//	LPC_C_CAN1->IF1_CMDREQ=msg;
//	while (LPC_C_CAN1->IF1_CMDREQ & IFCREQ_BUSY );
//	}else{
//	  LPC_C_CAN1->IF2_CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
//	  LPC_C_CAN1->IF2_CMDREQ=msg;
//	  while (LPC_C_CAN1->IF2_CMDREQ & IFCREQ_BUSY );
//  }
//
//}
//
...
//	if(IFsel == IF1){
//  LPC_C_CAN1->IF1_CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
//  LPC_C_CAN1->IF1_CMDREQ=msg;
//  while (LPC_C_CAN1->IF1_CMDREQ & IFCREQ_BUSY );
//	}else{
//		  LPC_C_CAN1->IF2_CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
//		  LPC_C_CAN1->IF2_CMDREQ=msg;
//		  while (LPC_C_CAN1->IF2_CMDREQ & IFCREQ_BUSY );
//	}
//}
...
void CAN_Init( uint32_t BitClk, CCAN_CLKDIV_Type ClkDiv , MSG_CB Tx_cb, MSG_CB Rx_cb)
{
	RX_cb = Rx_cb;
	TX_cb = Tx_cb;
	if ( !(LPC_C_CAN1->CNTL & CTRL_INIT) )
	{
		LPC_C_CAN1->CNTL |= CTRL_INIT;	
	}

	LPC_C_CAN1->CLKDIV = CLKDIV3;			
	LPC_C_CAN1->CNTL |= CTRL_CCE;	
	LPC_C_CAN1->BT = CAN_BITRATE500K12MHZ;
	LPC_C_CAN1->BRPE = 0x0000;
	LPC_C_CAN1->CNTL &= ~CTRL_CCE;		

	LPC_C_CAN1->CNTL &= ~CTRL_INIT;
	while ( LPC_C_CAN1->CNTL & CTRL_INIT );

	// By default, auto TX is enabled, enable all related interrupts 
	LPC_C_CAN1->CNTL |= (CTRL_IE|CTRL_SIE|CTRL_EIE);
