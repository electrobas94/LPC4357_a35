...
void I2C_Init(LPC_I2Cn_Type *I2Cx, uint32_t clockrate)
{
	...
	tem = CGU_GetPCLKFrequency(CGU_PERIPHERAL_M4CORE);
	tem	/= clockrate;
...
retry:
		...
		if ((TransferCfg->rx_length != 0) && (TransferCfg->rx_data != NULL)){
			...
			if ( CodeStatus != I2C_I2STAT_M_RX_SLAR_ACK )//CodeStatus != I2C_I2STAT_M_RX_DAT_ACK)//CodeStatus != I2C_I2STAT_M_RX_SLAR_ACK ||
			{
				TransferCfg->retransmissions_count++;
				if (TransferCfg->retransmissions_count > TransferCfg->retransmissions_max)
				{
